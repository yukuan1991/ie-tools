...
static bool dlg_verification(const std::string& serial)
{
    ...
    while(1)
    {
        ...
        /*向服务器发送get请求*/
//        auto str = http_get(UPDATE_SERVER_ADDR, "/user/selectUserByUserkey",
//        {{"userName", binary_to_base64 (account.toStdString ())}, {"serial", binary_to_base64 (serial)}}, 4000);
        auto str = http_get(UPDATE_SERVER_ADDR, "/user/selectUserByUserkey",
        {{"userName", account.toStdString ()}, {"serial", serial}}, 4000);
...
//    http_get(UPDATE_SERVER_ADDR, "/std-time/ip-log", {{"account", binary_to_base64(account)}}).data();
    return true;
}
catch(const std::exception &)
{
